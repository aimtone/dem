app.controller('tipo_de_tribunal', function($rootScope,$scope,$http,$q,$localStorage) {
	$rootScope.validateToken();
	$rootScope.objeto = "Tipo de Tribunal";
	// Variables predefinidas
	$scope.obj_padre = 'tipo_de_tribunal'; // Siempre terminar URL Con Simbolo "/"
	...
	angular.element(document).ready(function() {
		...
		setTimeout(function() {
			...
			$('#example').html("<thead><tr><th>Tipo de Tribunal</th></tr></thead>");
		...
		$scope.modal('.modal',false, null, function() { 
			...
			if($rootScope.actionButton=="ok") {
				...
				if($rootScope.button == "registrar") {
					...
					$rootScope.post('api/' + $scope.obj_padre, $scope.tipo_de_tribunal).then(function(response) {
				...
				if($rootScope.button == "modificar") {
					...
					$rootScope.put('api/' + $scope.obj_padre + '/' + $scope.datos.id, $scope.tipo_de_tribunal).then(function(response) {
						if(response!=null) {
							$scope.table.ajax.reload();
							$scope.datos = {};
							$rootScope.alert("Éxito", "Se ha completado el proceso de manera éxitosa", "success");
        		...
        		$rootScope.get('api/usuario?filter='+filter).then(function(response) {
        			console.log(response);
       				...
       				if(typeof response != "undefined") {
						...
						$rootScope.enviarNotificacion($rootScope.usuario_en_linea.nombres + " " + $rootScope.usuario_en_linea.apellidos + " HA ELIMINADO REGISTROS EN LA TABLA DE TIPO DE TRIBUNAL", "EL USUARIO " + $rootScope.usuario_en_linea.nombres + " " + $rootScope.usuario_en_linea.apellidos + " CUYA CEDULA DE IDENTIDAD ES " + $rootScope.usuario_en_linea.cedula + " HA ELIMINADO REGISTROS EN LA TABLA DE TIPO DE TRIBUNAL, POR FAVOR, VERIFIQUE ESTA INFORMACION EN LA BITACORA DEL SISTEMA PARA VER MAS DETALLES", "ADMINISTRADOR", null);
