app.controller('actividad', function($rootScope,$scope,$http,$q,$localStorage) {
	$rootScope.validateToken();
	$rootScope.objeto = "Actividad";
	// Variables predefinidas
	$scope.obj_padre = 'actividad'; // Siempre terminar URL Con Simbolo "/"
	...
	angular.element(document).ready(function() {
		...
		$scope.modal('.modal',false, null, function() { 
			...
			if($rootScope.actionButton=="ok") {
				...
				$scope.actividad = {
					id: null,
					actividad : $scope.datos.actividad,
				...
				if($rootScope.button == "registrar") {
					...
					$rootScope.post('api/' + $scope.obj_padre, $scope.actividad).then(function(response) {
				...
				if($rootScope.button == "modificar") {
					...
					$rootScope.put('api/' + $scope.obj_padre + '/' + $scope.datos.id, $scope.actividad).then(function(response) {
						if(response!=null) {
							$scope.table.ajax.reload();
							$scope.datos = {};
							$rootScope.alert("Éxito", "Se ha completado el proceso de manera éxitosa", "success");
        		...
        		$rootScope.get('api/usuario?filter='+filter).then(function(response) {
        			console.log(response);
       				...
       				if(typeof response != "undefined") {
						...
						$rootScope.enviarNotificacion($rootScope.usuario_en_linea.nombres + " " + $rootScope.usuario_en_linea.apellidos + " HA ELIMINADO REGISTROS EN LA TABLA DE ACTIVIDAD", "EL USUARIO " + $rootScope.usuario_en_linea.nombres + " " + $rootScope.usuario_en_linea.apellidos + " CUYA CEDULA DE IDENTIDAD ES " + $rootScope.usuario_en_linea.cedula + " HA ELIMINADO REGISTROS EN LA TABLA DE ACTIVIDAD, POR FAVOR, VERIFIQUE ESTA INFORMACION EN LA BITACORA DEL SISTEMA PARA VER MAS DETALLES", "ADMINISTRADOR", null);
	...
	$scope.closeModal = function(div,button) {
		if(button=='ok') {
			 ...
			 if($scope.datos.actividad==undefined) {  $rootScope.toast("Campo 'número' vacio"); return; }
