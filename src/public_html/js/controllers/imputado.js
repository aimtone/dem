app.controller('imputado', function($rootScope, $scope, $http, $q, $localStorage) {
    ...
    angular.element(document).ready(function() {
        ...
        setTimeout(function() {
            ...
            $('#example').html("<thead><tr><th>Cédula</th><th>Nombres</th><th>Apellidos</th><th>Email</th><th>Teléfono</th><th>Fecha de Nacimiento</th><th>Condición</th></tr></thead>");
                    ...
                    $rootScope.post('api/' + $scope.obj_padre, $scope.persona).then(function(response) {
                        if (response != null) {
                            ...
                            $rootScope.post('api/' + $scope.obj_hijo, $scope.obj).then(function(response) {
                                if (response != null) {
                                    $rootScope.post('api/persona_tipo', { cedula: $scope.persona.cedula, tipo_persona: 4 }).then(function(response) {
                                ...
                                $rootScope.delete('api/' + $scope.obj_hijo + '/' + id).then(function(response) {
                                    if (response != null) {
                                        var filter_2 = JSON.stringify({
                                            donde: "WHERE cedula = '" + response["0"].cedula + "' AND tipo_persona = 4"
                                        });
    ...
    $scope.closeModal = function(div, button) {
        if (button == 'ok') {
            ...
            if ($scope.datos.condicion == undefined) { $rootScope.toast("Selecciona una condicion"); return; }
