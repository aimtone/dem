app.controller('defensor', function($rootScope, $scope, $http, $q, $localStorage) {
    ...
    $scope.obj_hijo = 'defensor'; // Siempre terminar URL Con Simbolo "/"
        ...
        $scope.modal('.modal', false, null, function() {
            ...
            if ($rootScope.actionButton == "ok") {
                ...
                $scope.obj = {
                    id: null,
                    cedula: $scope.datos.cedula.toUpperCase(),
                    tipo: $scope.datos.tipo.toUpperCase(),
                    ...
                    $rootScope.post('api/' + $scope.obj_padre, $scope.persona).then(function(response) {
                        if (response != null) {
                            ...
                            $rootScope.post('api/' + $scope.obj_hijo, $scope.obj).then(function(response) {
                                if (response != null) {
                                    $rootScope.post('api/persona_tipo', { cedula: $scope.persona.cedula, tipo_persona: 1 }).then(function(response) {
            ...
            function(response) {
                ...
                $rootScope.get('api/usuario?filter=' + filter).then(function(response) {
                    ...
                    if (typeof response != "undefined") {
                                ...
                                $rootScope.delete('api/' + $scope.obj_hijo + '/' + id).then(function(response) {
                                    if (response != null) {
                                        var filter_2 = JSON.stringify({
                                            donde: "WHERE cedula = '" + response["0"].cedula + "' AND tipo_persona = 1"
                                        });
                        ...
                        $rootScope.enviarNotificacion($rootScope.usuario_en_linea.nombres + " " + $rootScope.usuario_en_linea.apellidos + " HA ELIMINADO REGISTROS EN LA TABLA DE DEFENSOR", "EL USUARIO " + $rootScope.usuario_en_linea.nombres + " " + $rootScope.usuario_en_linea.apellidos + " CUYA CEDULA DE IDENTIDAD ES " + $rootScope.usuario_en_linea.cedula + " HA ELIMINADO REGISTROS EN LA TABLA DE DEFENSOR, POR FAVOR, VERIFIQUE ESTA INFORMACION EN LA BITACORA DEL SISTEMA PARA VER MAS DETALLES", "ADMINISTRADOR", null);
