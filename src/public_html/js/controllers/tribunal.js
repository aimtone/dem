app.controller('tribunal', function($rootScope, $scope, $http, $q, $localStorage) {
    $rootScope.validateToken();
    $rootScope.objeto = "Tribunal";
    // Variables predefinidas
    $scope.obj_padre = 'tribunal'; // Siempre terminar URL Con Simbolo "/"
        ...
        $scope.modal('.modal', false, null, function() {
            ...
            if ($rootScope.actionButton == "ok") {
                ...
                if ($rootScope.button == "registrar") {
                    ...
                    $rootScope.post('api/' + $scope.obj_padre, $scope.tribunal).then(function(response) {
                ...
                if ($rootScope.button == "modificar") {
                    ...
                    $rootScope.put('api/' + $scope.obj_padre + '/' + $scope.datos.id, $scope.tribunal).then(function(response) {
                        if (response != null) {
                            $scope.table.ajax.reload();
                            $scope.datos = {};
                            $rootScope.alert("Éxito", "Se ha completado el proceso de manera éxitosa", "success");
    ...
    // -----------------------------------------------------------------
    var filter = JSON.stringify({
        ordenarPor: "order by descripcion"
    });
    $scope.tConfig = {
        language: {
        ...
        ajax: {
            url: 'api/vista_' + $scope.obj_padre + "?filter=" + filter,
            headers: {
