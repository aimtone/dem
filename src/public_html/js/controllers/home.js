		app.controller('home', function($rootScope, $scope, $http, $localStorage, $location) {
		    ...
		    $scope.grafico = {
		        label: "ESTATUS",
		        titulo: "CANTIDAD DE ACTOS POR ESTATUS",
		        subtitulo: "",
		        tipo: "bar",
		        agrupar: "ESTATUS",
		    ...
		    $scope.tablas = [{
		        ...
		        {
		            nombre: "ALGUACIL",
		            nombre_real: "ALGUACIL"
		        },
		        {
		            nombre: "DEFENSOR",
		            nombre_real: "DEFENSOR"
		        },
		        {
		            nombre: "VICTIMA",
		            nombre_real: "VICTIMA"
		        ...
		        },
		        {
		            ...
		            nombre_real: "HORA_INICIO"
		        },
		        {
		            nombre: "HORA DE FIN",
		            nombre_real: "HORA_FIN"
		        },
		        {
		            nombre: "SALA",
		            nombre_real: "SALA"
		    ...
		    */
		    ...
		    $rootScope.cargarChart = function(titulo, pocision, etiquetas, agrupar, representacion) {
		        var labels = [];
		        var data = [];

		        if ((typeof $scope.fechas.fecha_inicial != "undefined" && $scope.fechas.fecha_inicial != null) && (typeof $scope.fechas.fecha_final != "undefined" && $scope.fechas.fecha_final != null)) {
		            var filter1 = JSON.stringify({
		                campos: "COUNT(*) AS cantidad",
		                donde: "WHERE FECHA BETWEEN '" + moment($scope.fechas.fecha_inicial).format("YYYY-MM-DD") + "' AND '" + moment($scope.fechas.fecha_final).format("YYYY-MM-DD") + "'"
		            });
		        } else {
		            var filter1 = JSON.stringify({
		                campos: "COUNT(*) AS cantidad"
		        ...
		        $rootScope.get('api/vista_estadistica?filter=' + filter1).then(function(response) {
		            ...
		            $rootScope.get('api/vista_estadistica?filter=' + filter).then(function(response) {
		                ...
		                if ((typeof $scope.fechas.fecha_inicial != "undefined" && $scope.fechas.fecha_inicial != null) && (typeof $scope.fechas.fecha_final != "undefined" && $scope.fechas.fecha_final != null)) {
		                    ...
		                    var object = {
		                        title: titulo,
		                        subtitle: "DESDE " + moment($scope.fechas.fecha_inicial).format("DD/MM/YYYY") + " HASTA " + moment($scope.fechas.fecha_final).format("DD/MM/YYYY"),
		                        labels: labels,
		                        data: data
		                    };
		                ...
		                $scope.grafico.subtitulo = object.subtitle;
		    ...
		    $rootScope.cargarChart($scope.grafico.titulo, 0, $scope.grafico.label, $scope.grafico.agrupar, $scope.grafico.representacion);

		    var date = moment().format("YYYY-MM-DD");
		    $rootScope.cargarEventos(date);

		    $rootScope.cargarEjeX = function(array) {
		        if (array == "vista_estadistica") {
		    ...
		    $rootScope.generarPDF = function() {
		        ...
		        $rootScope.get('api/vista_acto?filter=' + filter).then(function(response) {
		            ...
		            var table_body = [
		                ['ID', 'CAUSA', 'IMPUTADO', 'TRIBUNAL', 'TIPO DE TRIBUNAL', 'FISCAL', 'DEFENSOR', 'ACTO', 'HORA', 'ALGUACIL', 'SALA', 'OBSERVACION']
		            ];
